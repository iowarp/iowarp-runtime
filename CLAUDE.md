...
- Implement `Aggregate()` for tasks that need to combine results from multiple replicas
- Skip both for local-only tasks or tasks with no output parameters

**Note:** The autogenerated dispatcher methods in `*_lib_exec.cc` call these task-level methods automatically
