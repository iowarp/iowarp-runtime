...
struct CreateParams {
  ...
  chi::u32 io_depth_;              // libaio queue depth
  ...
  explicit CreateParams(const hipc::CtxAllocator<CHI_MAIN_ALLOC_T> &alloc)
      : total_size_(0), io_depth_(32), alignment_(4096) {}
  ...
  CreateParams(const hipc::CtxAllocator<CHI_MAIN_ALLOC_T> &alloc, const CreateParams& other)
      : file_path_(other.file_path_), total_size_(other.total_size_), 
  ...
  template<class Archive>
  void serialize(Archive& ar) {
    ar(file_path_, total_size_, io_depth_, alignment_);
  }
};
...
/**
 * AllocateTask - Allocate a data block with specified size
 */
struct AllocateTask : public chi::Task {
  // Task-specific data
  IN chi::u64 size_;           // Requested block size
  OUT Block block_;            // Allocated block information
  ...
  explicit AllocateTask(const hipc::CtxAllocator<CHI_MAIN_ALLOC_T> &alloc) 
      : chi::Task(alloc), size_(0), result_code_(0) {}
  ...
  void SerializeOut(Archive& ar) {
    ar(block_, result_code_);
  }
};

/**
 * FreeTask - Free an allocated data block
 */
