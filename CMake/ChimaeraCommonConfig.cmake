...
macro(add_chimod_library namespace target)
    ...
    if (CHIMAERA_ENABLE_CUDA)
        add_cuda_library(${target} SHARED TRUE ${ARGN})
        target_link_libraries(${target} PUBLIC hshm::cudacxx)
        target_compile_definitions(${target} PUBLIC CHIMAERA_ENABLE_CUDA)
    elseif (CHIMAERA_ENABLE_ROCM)
        add_rocm_gpu_library(${target} SHARED TRUE ${ARGN})
        target_link_libraries(${target} PUBLIC hshm::rocmcxx_gpu)
        target_compile_definitions(${target} PUBLIC CHIMAERA_ENABLE_ROCM)
    else()
    ...
    target_link_libraries(${target}_client INTERFACE chimaera::client)
    ...
    if (CHIMAERA_ENABLE_CUDA)
        ...
        target_link_libraries(${target}_client_gpu INTERFACE hshm::cudacxx chimaera::client_gpu)
    ...
    if (CHIMAERA_ENABLE_ROCM)
        add_library(${target}_client_gpu INTERFACE)
        target_link_libraries(${target}_client_gpu INTERFACE hshm::rocmcxx_gpu chimaera::client_gpu)
        target_include_directories(${target}_client_gpu INTERFACE ${CMAKE_INSTALL_PREFIX}/include)
        list(APPEND ${namespace}_${target}_exports ${target}_client_gpu)
    endif()
endmacro()
