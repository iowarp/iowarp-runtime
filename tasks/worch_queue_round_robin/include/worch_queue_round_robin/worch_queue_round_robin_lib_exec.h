#ifndef CHI_WORCH_QUEUE_ROUND_ROBIN_LIB_EXEC_H_
#define CHI_WORCH_QUEUE_ROUND_ROBIN_LIB_EXEC_H_
...
void Del(const hipc::MemContext &mctx, u32 method, Task *task) override {
  switch (method) {
    ...
    case Method::kSchedule: {
      CHI_CLIENT->DelTask<ScheduleTask>(mctx,
                                        reinterpret_cast<ScheduleTask *>(task));
      break;
    }
  }
}
...
void CopyStart(u32 method, const Task *orig_task, Task *dup_task,
  ...
  switch (method) {
    ...
    case Method::kSchedule: {
      chi::CALL_COPY_START(reinterpret_cast<const ScheduleTask *>(orig_task),
  ...
  switch (method) {
    ...
    case Method::kSchedule: {
      chi::CALL_NEW_COPY_START(
          reinterpret_cast<const ScheduleTask *>(orig_task), dup_task, deep);
      break;
    }
  }
}
...
TaskPointer LoadStart(u32 method, BinaryInputArchive<true> &ar) override {
  ...
  switch (method) {
    ...
    case Method::kSchedule: {
      task_ptr.ptr_ =
          CHI_CLIENT->NewEmptyTask<ScheduleTask>(HSHM_MCTX, task_ptr.shm_);
