...
void Dup(u32 method, Task *orig_task, std::vector<LPointer<Task>> &dups) override {
  switch (method) {
    ...
    case Method::kCustom: {
      chm::CALL_DUPLICATE(reinterpret_cast<CustomTask*>(orig_task), dups);
      break;
    }
  }
}
...
void DupEnd(u32 method, u32 replica, Task *orig_task, Task *dup_task) override {
  switch (method) {
    ...
    case Method::kCustom: {
      chm::CALL_DUPLICATE_END(replica, reinterpret_cast<CustomTask*>(orig_task), reinterpret_cast<CustomTask*>(dup_task));
      break;
    }
  }
}
...
void ReplicateStart(u32 method, u32 count, Task *task) override {
  switch (method) {
    ...
    case Method::kCustom: {
      chm::CALL_REPLICA_START(count, reinterpret_cast<CustomTask*>(task));
      break;
    }
  }
}
...
void ReplicateEnd(u32 method, Task *task) override {
  switch (method) {
    ...
    case Method::kCustom: {
      chm::CALL_REPLICA_END(reinterpret_cast<CustomTask*>(task));
      break;
    }
  }
}
...
bool CompareGroup(u32 method, Task *task1, Task *task2) override {
  switch (method) {
    ...
    case Method::kCustom: {
      return chm::CALL_CMPGRP<CustomTask>(
        reinterpret_cast<CustomTask*>(task1), task2);
    }
  }
  return false;
}
