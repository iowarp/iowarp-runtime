...
void Task::Wait() {
  ...
  if (container) {
    ...
    try {
      container->Monitor(MonitorModeId::kEstLoad, method_, 
                        run_ctx->current_task, est_ctx);
      
      // The estimated time should be stored in est_ctx.estimated_completion_time_us
      run_ctx->estimated_completion_time_us = est_ctx.estimated_completion_time_us;
  ...
  if (worker) {
    worker->AddToBlockedQueue(run_ctx->current_task, run_ctx, run_ctx->estimated_completion_time_us);
  }
