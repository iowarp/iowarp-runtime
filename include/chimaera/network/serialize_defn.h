...
struct TaskSegment {
  ...
  TaskSegment(PoolId task_state, u32 method, size_t task_addr,
      ...
      : pool_(task_state), method_(method), task_addr_(task_addr), dom_(dom) {}
  ...
  friend std::ostream &operator<<(std::ostream &os, const TaskSegment &task) {
    os << hshm::Formatter::format("TaskSeg[{}]: pool={} method={} dom={}",
                                  task.task_addr_, task.pool_, task.method_,
                                  task.dom_);
    return os;
  }
};
...
class BinaryOutputArchive {
 ...
 public:
    ...
    if constexpr (IS_TASK(T)) {
      if constexpr (IS_SRL(T)) {
        if constexpr (is_start) {
          ...
          xfer_.tasks_.emplace_back(var.pool_, var.method_, (size_t)&var,
                                    var.dom_query_);
  ...
  BinaryOutputArchive &Serialize() { return *this; }
...
class BinaryInputArchive {
 ...
 public:
  ...
  BinaryInputArchive &bulk(chi::IntFlag flags, hipc::Pointer &data,
    ...
    if constexpr (is_start) {
      data = HSHM_MEMORY_MANAGER->Convert<void, hipc::Pointer>(xfer_data);
    }
    return *this;
  }
  ...
  HSHM_INLINE
  BinaryInputArchive &Deserialize() { return *this; }
};
