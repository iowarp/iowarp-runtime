...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    if (!chi::CHIMAERA_CLIENT_INIT()) {
      std::cerr << "Failed to initialize Chimaera client components" << std::endl;
      return 1;
    }
    ...
    if (!ipc_manager || !ipc_manager->IsInitialized()) {
      std::cerr << "IPC manager not available - is Chimaera runtime running?" << std::endl;
      return 1;
    }
    ...
    if (!task_queue || task_queue->IsNull()) {
      std::cerr << "TaskQueue not available - runtime may not be properly initialized" << std::endl;
      return 1;
    }
    ...
    try {
      ...
      if (num_lanes == 0) {
        std::cerr << "TaskQueue has no lanes configured - runtime initialization incomplete" << std::endl;
        return 1;
      }
      std::cout << "TaskQueue validated with " << num_lanes << " lanes" << std::endl;
    ...
    try {
      stop_task = admin_client.AsyncStopRuntime(HSHM_DEFAULT_MEM_CTX, dom_query, 
                                                shutdown_flags, grace_period_ms);
      if (stop_task.IsNull()) {
        std::cerr << "Failed to create stop runtime task - runtime may not be running" << std::endl;
        return 1;
      }
    } catch (const std::exception& e) {
      std::cerr << "Error creating stop runtime task: " << e.what() << std::endl;
      return 1;
    }
    
    std::cout << "Stop runtime task submitted successfully (fire-and-forget)" << std::endl;
    
    auto end_time = std::chrono::steady_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(
        end_time - start_time).count();
    
    std::cout << "Runtime stop task submitted in " 
